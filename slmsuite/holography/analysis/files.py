        ...
        for key in data.keys():
            if isinstance(data[key], dict):
                ...
                try:
                    ...
                    raise ValueError(
                        f"save_h5() does not support saving staggered arrays such as {str(data[key])}. "
                        f"Arrays must be uniform. {str(e)}"
                    )
...
def save_image(
    file_path, images, cmap=False, lut=None, normalize=True, border=None, **kwargs
):
    """
