...
class MindVision(Camera):
    ...
    def __init__(self, serial="", pitch_um=None, verbose=True, **kwargs):
        ...
        **kwargs
        ...
        # Turn the camera on.
        if verbose:
            print(f"Initializing sn '{serial}'...", end="")
        self.handle = 0
        try:
        ...
        if self.mono:
            ...
            warnings.warn(
                "Camera is not grayscale. Color cameras may cause issues in slmsuite."
            )
    ...
    def _get_image_hw(self, timeout_s):
        ...
        try:
            #
            raw_data, frame_head = _mvsdk.CameraGetImageBuffer(
                self.handle, int(timeout_s)
            )
            ...
            if self.mono:
                ...
                return np.copy(
                    np.frombuffer(frame_data, dtype=np.uint8).reshape(rgb_shape)
                )
            else:
                return np.copy(
                    np.frombuffer(frame_data, dtype=np.uint8).reshape(self.shape)
                )

        except _mvsdk.CameraException as e:
            print(
                f"CameraGetImageBuffer failed ({e.error_code}):\n{e.message}"
            )
