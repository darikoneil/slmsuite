...
images_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "../_build/html/_images"
)
...
def setup(app):
    ...
    try:
        ...
        tree_url = (
            "https://api.github.com/repos/{}/{}/git/trees/main?recursive=1".format(
        ...
        for path_object in tree_response["tree"]:
            ...
            if path_str[0:9] == "examples/" and (
                (path_str[-6:] == ".ipynb") or (path_str[-4:] == ".gif")
            ):
                ...
                file_url2 = "https://github.com/{}/{}/blob/main/{}?raw=true".format(
                    examples_repo_owner, examples_repo_name, path_str
                )
                if path_str[-6:] == ".ipynb":
                    file_path = os.path.join(examples_path, file_name)
                    file_response = requests.get(file_url).json()
                    file_content = file_response["content"]
                    file_str = base64.b64decode(file_content.encode("utf8")).decode(
                        "utf8"
                    )
